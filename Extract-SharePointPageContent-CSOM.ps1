<#
.SYNOPSIS
    SharePoint 2016 Client-Side Page Content Extractor with MFA Support
    
.DESCRIPTION
    This script extracts complete page information from SharePoint 2016 using Client-Side Object Model (CSOM)
    from a remote client machine with Multi-Factor Authentication (MFA) support.
    
    Features:
    - Remote client execution (no need to run on SharePoint server)
    - Multi-Factor Authentication (MFA) support
    - All page metadata extraction
...
.PARAMETER SiteUrl
    The URL of the SharePoint site (e.g., "https://contoso.sharepoint.com" or "http://sharepoint.contoso.com")
...
.PARAMETER UseInteractiveAuth
    Use interactive authentication (recommended for MFA)
    
.PARAMETER Credentials
    Optional traditional credentials (not recommended for MFA environments)
    
.PARAMETER CSOMPath
    Path to CSOM assemblies (if not in default location)
    
.EXAMPLE
    # Interactive authentication with MFA support
    .\Extract-SharePointPageContent-CSOM.ps1 -SiteUrl "https://contoso.sharepoint.com" -PageUrl "/Pages/Home.aspx" -OutputPath "C:\Temp\PageData.xml" -UseInteractiveAuth
    
.EXAMPLE
    # On-premises with traditional authentication
    $creds = Get-Credential
    .\Extract-SharePointPageContent-CSOM.ps1 -SiteUrl "http://sharepoint.contoso.com" -PageUrl "/Pages/Home.aspx" -OutputPath "C:\Temp\PageData.xml" -Credentials $creds
    
.EXAMPLE
    # Custom CSOM path
    .\Extract-SharePointPageContent-CSOM.ps1 -SiteUrl "https://contoso.sharepoint.com" -PageUrl "/Pages/Home.aspx" -OutputPath "C:\Temp\PageData.xml" -UseInteractiveAuth -CSOMPath "C:\CSOM"
#>
...
function Load-CSOMAssemblies {
    ...
    foreach ($path in $possiblePaths) {
        if (Test-Path $path) {
            ...
            foreach ($assembly in $requiredAssemblies) {
                $assemblyFile = Join-Path $path $assembly
    ...
    if (-not $assemblyPath) {
        throw "CSOM assemblies not found. Please install SharePoint Client Components or specify the path with -CSOMPath parameter."
    }
    ...
    foreach ($assembly in $requiredAssemblies) {
        ...
        try {
            ...
            throw "Failed to load required assembly: $assembly. Error: $($_.Exception.Message)"
        }
    }
    
    # Load optional assemblies
    foreach ($assembly in $optionalAssemblies) {
        $assemblyFile = Join-Path $assemblyPath $assembly
        if (Test-Path $assemblyFile) {
            try {
                ...
                Write-Log "⚠ Could not load optional assembly: $assembly" "Warning"
            }
        }
    }
...
function Initialize-Authentication {
    ...
    Write-Log "Initializing authentication..." "Info"
    
    try {
        $context = New-Object Microsoft.SharePoint.Client.ClientContext($SiteUrl)
        
        if ($UseInteractive) {
            Write-Log "Using interactive authentication (supports MFA)..." "Info"
            
            # For SharePoint Online with MFA
            if ($SiteUrl.Contains(".sharepoint.com")) {
                Write-Log "Detected SharePoint Online - using modern authentication" "Info"
                
                # Use Windows Authentication Manager for interactive auth
                # This will trigger the modern auth dialog
                $authManager = [Microsoft.SharePoint.Client.ClientContext]::new($SiteUrl)
                
                # For SharePoint Online, we can use the ExecutingWebRequest event to handle auth
                $authHandler = {
                    param($sender, $e)
                    Write-Log "Authentication dialog may appear..." "Info"
                }
                ...
                try {
                    ...
                    if ($_.Exception.Message -like "*401*" -or $_.Exception.Message -like "*Unauthorized*") {
                        Write-Log "Authentication required. Please complete the authentication in the browser window." "Warning"
                        
                        # Try to use WebAuthenticationBroker or similar modern auth
                        $credential = [Microsoft.SharePoint.Client.SharePointOnlineCredentials]::new($null, $null)
                        ...
                        try {
                            ...
                            throw "Interactive authentication failed. Please ensure you have the latest SharePoint Client Components installed and try again."
                        }
                    }
                    else {
                        throw
                ...
                # On-premises SharePoint - use default credentials or prompt
                Write-Log "Detected on-premises SharePoint - using Windows authentication" "Info"
                $context.Credentials = [System.Net.CredentialCache]::DefaultCredentials
                
                try {
                    $context.Load($context.Web)
                    $context.ExecuteQuery()
                    Write-Log "✓ Windows authentication successful" "Success"
                }
                catch {
                    Write-Log "Windows authentication failed, prompting for credentials..." "Warning"
                    $creds = Get-Credential -Message "Enter SharePoint credentials"
                    $context.Credentials = $creds.GetNetworkCredential()
                    
                    $context.Load($context.Web)
                    $context.ExecuteQuery()
                    Write-Log "✓ Credential authentication successful" "Success"
                }
            }
        }
        elseif ($Creds) {
            ...
            if ($SiteUrl.Contains(".sharepoint.com")) {
                # SharePoint Online with username/password
            ...
            Write-Log "Using default authentication..." "Info"
            $context.Credentials = [System.Net.CredentialCache]::DefaultCredentials
            
            $context.Load($context.Web)
            $context.ExecuteQuery()
            Write-Log "✓ Default authentication successful" "Success"
        }
        ...
        if ($_.Exception.Message -like "*401*" -or $_.Exception.Message -like "*Unauthorized*") {
            Write-Log "Authentication Error: Please check your credentials or use -UseInteractiveAuth for MFA support" "Error"
        }
...
function Extract-PageContent {
    ...
    try {
        # For CSOM, we can't easily get the raw file content like in server-side code
        # But we can extract the content from list item fields
...
function Extract-SharePointPageData {
    ...
    Write-Log "Starting SharePoint page extraction using CSOM..." "Info"
    ...
    try {
        ...
        $context = Initialize-Authentication -SiteUrl $SiteUrl -UseInteractive $UseInteractive -Creds $Creds
        ...
        $extractionInfo = @{
            ScriptVersion = "1.1.0"
            SharePointVersion = "SharePoint 2016 (CSOM)"
            ExtractionUser = [Environment]::UserName
            ExtractionMethod = "PowerShell CSOM Client-Side"
            Timestamp = Get-Date
        }
...
#region XML Generation (Same as original)
...
<SharePointPageData xmlns="http://schemas.sharepoint.com/page-extraction/2016" version="1.1" extractionDate="$(Get-Date -Format 'yyyy-MM-ddTHH:mm:ss')">
  <ExtractionInfo>
...
try {
    Write-Log "SharePoint 2016 CSOM Page Content Extractor v1.1 (MFA Support)" "Info"
    Write-Log "================================================================" "Info"
    
    # Load CSOM assemblies
    $loadedAssemblies = Load-CSOMAssemblies -CustomPath $CSOMPath
    Write-Log "Loaded $($loadedAssemblies.Count) CSOM assemblies" "Success"
    
    # Validate parameters
    if (-not $UseInteractiveAuth -and -not $Credentials) {
        Write-Log "Warning: No authentication method specified. Will try default authentication." "Warning"
        Write-Log "For MFA environments, use -UseInteractiveAuth parameter." "Warning"
    }
    
    # Extract page data
    $extractedData = Extract-SharePointPageData -SiteUrl $SiteUrl -PageUrl $PageUrl -UseInteractive $UseInteractiveAuth.IsPresent -Creds $Credentials
    ...
    if ($_.Exception.Message -like "*401*" -or $_.Exception.Message -like "*Unauthorized*") {
        Write-Log "" "Error"
        Write-Log "AUTHENTICATION TROUBLESHOOTING:" "Error"
        Write-Log "- For MFA environments, use: -UseInteractiveAuth" "Error"
        Write-Log "- For SharePoint Online, ensure you have modern authentication enabled" "Error"
        Write-Log "- For on-premises, check your credentials and permissions" "Error"
    }
    elseif ($_.Exception.Message -like "*CSOM*" -or $_.Exception.Message -like "*assembly*") {
        Write-Log "" "Error"
        Write-Log "CSOM TROUBLESHOOTING:" "Error"
        Write-Log "- Install SharePoint Client Components SDK" "Error"
        Write-Log "- Download from: https://www.microsoft.com/en-us/download/details.aspx?id=35585" "Error"
        Write-Log "- Or specify custom path with: -CSOMPath 'C:\\Path\\To\\CSOM'" "Error"
    }
